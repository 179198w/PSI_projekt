		...
		<form:form method="post" enctype="multipart/form-data" class="form-horizontal" commandName="priceEditCommand">
			<div class="long-panel panel panel-primary">
				...
				<div class="panel-body">
					...
					<spring:bind path="periodId">
						<div class="form-group ${status.error ? 'has-error' : ''}">
							<form:label path="periodId" class="col-md-4 control-label">Termin:</form:label>
							<div class="col-md-4">
								<form:select path="periodId" class="form-control">									
									<c:forEach items="${periods}" var="period">
										<form:option value="${period.id}">${period.from} - ${period.to}</form:option>
									</c:forEach>
								</form:select>
								<form:errors path="periodId" cssClass="help-block" />
							</div>
						</div>
					</spring:bind>
					<spring:bind path="touristEventComponentId">
						<div id="touristEventComponent" ${priceEditCommand.touristEventComponentId == null ? "style=\"display: none\"" : ""} class="form-group ${status.error ? 'has-error' : ''}">
							...
							<div class="col-md-4">
								<form:select path="touristEventComponentId" class="form-control">
									...
									<c:forEach items="${touristEventComponents}" var="touristEventComponent">
										<form:option value="${touristEventComponent.id}">${touristEventComponent.type.name} - ${touristEventComponent.name}</form:option>
									</c:forEach>
								</form:select>
